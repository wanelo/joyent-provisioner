describe Provisioner::Command::Knife do
  ...
  describe '#shell_commands' do
    context 'host is specified' do
      let(:subject) { Provisioner::Command::Knife.new(template_configuration, number: '1') }
    ...
    context 'host number is not specified' do
      let(:subject) { Provisioner::Command::Knife.new(template_configuration) }
      let(:template_configuration) { configuration.for_template('memcached-sessions') }
